          ...
          try {
            ...
            // CASE 2: Handle bbox format [x1, y1, x2, y2]
              ...
              if (Array.isArray(detection.bbox)) {
                // Array format
                const bboxArray = detection.bbox as number[];
                if (bboxArray.length >= 4) {
                  [x1, y1, x2, y2] = bboxArray;
                } else {
                  console.warn(`Invalid bbox array length: ${bboxArray.length}`, detection);
                  return null;
                }
              ...
              // Verify all coordinates are defined before proceeding
              if (x1 === undefined || y1 === undefined || x2 === undefined || y2 === undefined) {
                console.warn('Undefined coordinates after extraction', detection);
                return null;
              }
